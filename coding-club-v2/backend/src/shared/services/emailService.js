...
function getClient() {
  if (!apiInstance) {
    if (!config.brevo.apiKey) {
      console.warn('[Email] BREVO_API_KEY not set. Emails will be skipped.');
      return null;
    }
    apiInstance = new brevo.TransactionalEmailsApi();
    apiInstance.setApiKey(brevo.TransactionalEmailsApi.ApiKeys.apiKey, config.brevo.apiKey);
  }
  return apiInstance;
}

function getSender() {
  return { email: config.brevo.senderEmail, name: config.brevo.senderName };
}
...
async function sendEmail(to, subject, htmlContent) {
  ...
  try {
    ...
    console.log(`[Email] Sent to ${to.email}: ${subject} (messageId: ${result.body.messageId})`);
    return result.body;
  } catch (err) {
    ...
    <body style="font-family: 'Segoe UI', Arial, sans-serif; background: #0f0f0f; color: #e0e0e0; padding: 40px 20px;">
      <div style="max-width: 520px; margin: 0 auto; background: #1a1a2e; border-radius: 16px; padding: 40px; border: 1px solid #2a2a4a;">
        ...
        <p style="font-size: 15px; line-height: 1.7; color: #bbb;">
          We're thrilled to have you on board! CodeAD is your platform to sharpen your coding skills,
          compete on the leaderboard, and grow as a developer.
        </p>
        ...
        <a href="${config.frontendUrl}/problems" style="display: inline-block; background: #f59e0b; color: #000; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; margin: 16px 0;">
          Start Solving Problems
        </a>

        <p style="font-size: 13px; color: #666; margin-top: 32px; border-top: 1px solid #2a2a4a; padding-top: 16px;">
          Happy Coding!<br><strong style="color: #f59e0b;">Team CodeAD</strong>
        </p>
      </div>
    </body>
    </html>
  ...
  const htmlContent = `
    ...
    <body style="font-family: 'Segoe UI', Arial, sans-serif; background: #0f0f0f; color: #e0e0e0; padding: 40px 20px;">
      <div style="max-width: 520px; margin: 0 auto; background: #1a1a2e; border-radius: 16px; padding: 40px; border: 1px solid #2a2a4a;">
        ...
        <p style="font-size: 15px; line-height: 1.7; color: #bbb;">
          A new coding problem has been uploaded to CodeAD. Give it a try!
        </p>
        ...
        <div style="background: #16213e; border-radius: 12px; padding: 24px; margin: 24px 0; border: 1px solid #2a2a4a;">
          ...
          <p style="margin: 0 0 12px 0;">
            <span style="background: ${color}; color: #000; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase;">${problem.difficulty}</span>
          </p>
          <p style="font-size: 14px; color: #bbb; line-height: 1.6; margin: 0;">${problem.description}</p>
        </div>

        <a href="${config.frontendUrl}/problems/${problem.id}" style="display: inline-block; background: #f59e0b; color: #000; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; margin: 16px 0;">
          Solve Now
        </a>
